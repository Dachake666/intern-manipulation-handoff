...
使用方式与 collect_wrist_orientation_samples.py 一致：
  * 网络身份只从 CLI 或环境变量注入，不修改源码。
  * ARM_IP 默认跟随 ROBOT_IP，也可在会话配置中单独提供。
...
安全边界：
  ...
  * 全程持有 robot_lock.py 的机器人级互斥锁，避免任何另一臂或夹爪进程并发。
...
# ---------------------------------------------------------------- 配置
# 网络身份只从 CLI/环境注入，禁止把 .147/.148 固化进候选工具。
...
STEP_MM = 20.0                   # 新候选步长；已成功真机基线是 10mm
...
def _run_robot_locked() -> int:
    ...
    try:
        ...
        if not ENABLE_REAL_MOTION:
            ...
            print("真机需显式设置 XIFENG_ALLOW_REAL_MOTION=1 并同时给 --run。")
            return 0
...
def run_robot() -> int:
    _validate_config()
    with robot_lock.acquire_robot(ROBOT_IP, "execute_worlds_10x.py"):
        return _run_robot_locked()


def main(argv=None) -> int:
    global ROBOT_IP, LOCAL_IP, ARM_IP, ARM_PORT, STEP_MM, GLOBAL_SPEED
    global ENABLE_REAL_MOTION
    ap = argparse.ArgumentParser(description="左臂 worlds 十步候选（运行时网络配置）")
    ...
    ap.add_argument("--run", action="store_true", help="请求真机运动；仍需源码安全开关允许")
    a = ap.parse_args(argv)
    ROBOT_IP, LOCAL_IP, ARM_IP, ARM_PORT = a.robot_ip, a.local_ip, (a.arm_ip or a.robot_ip), a.arm_port
    STEP_MM, GLOBAL_SPEED = a.step_mm, a.speed
    source_gate = bool(ENABLE_REAL_MOTION)
    ENABLE_REAL_MOTION = bool(a.run and source_gate)
    if a.run and not source_gate:
        print("✗ --run 已给出，但 ENABLE_REAL_MOTION=False；拒绝下发")
        return 2
    return run_robot()
