...
def assert_start_safe(current: Mapping[str, Sequence[float]], plan: Mapping[str, Any],
    ...
    if start_policy == "nominal-safe" and (dp > START_POS_TOL_MM or da > START_UVW_TOL_DEG):
        raise RuntimeError(
    ...
    *,
    ...
    for stage in effective_stages(plan):
        ...
        if kind == "ASSERT_ENDPOINT":
            ...
            require_protection(sdk)
            assert_unchanged(snapshot(sdk), event["before"], f"pre-command/{name}")
            ...
            if name == "PICK_ASCEND":
                ...
                assert_unchanged(snapshot(sdk), {"worlds": actual_world, "joints": event["arrival_joints"]},
                                 "PICK_ASCEND operator pause")
            ...
            if name == "PLACE_HOVER":
                ...
                operator_gate("At PLACE_HOVER. Inspect bottle/table/frame clearance before descent",
                              input_fn, auto_continue)
                assert_unchanged(snapshot(sdk), {"worlds": actual_world, "joints": actual_q},
                                 "PLACE_HOVER operator pause")
        else:
...
def runtime_config(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not math.isfinite(speed) or not 0.1 <= speed <= 5.0:
        raise ValueError("--speed must be in [0.1,5.0] for this changed-executor trial")
...
def main(argv: Sequence[str] | None = None, *, input_fn: Any = input) -> int:
    ...
    try:
        ...
        try:
            qualification = qualification_gate(plan, plan_hash, args)
            log["qualification"] = qualification
        except (ValueError, RuntimeError, OSError, KeyError) as exc:
            log["qualification"] = {"status": "BLOCKED", "reason": str(exc)}
            if args.run:
                raise
        if args.dry_run:
            ...
            with robot_lock.acquire(config["robot_ip"], ARM_ID, "execute_tabletop_hybrid_trial.py"):
                ...
                try:
                    ...
                    log["status"] = "READONLY_HARD_PRECHECK_PASS_NOT_MOTION_AUTHORIZED"
                    print(f"READ-ONLY IK/LIMIT PRECHECK PASS: {len(pre['segments'])} motion segments")
                    if args.run:
                        ...
                        require_qualified_start(plan, reviewed)
                        ...
                        log["enabled_first_world_precheck"] = dense_world_precheck(
