...
try:
    import pypilot
except ModuleNotFoundError:  # 允许开发机执行 --help、dry-run 与纯离线测试。
    pypilot = None

# ---------------------------------------------------------------- 限位表
# SDK 关节序号 1..7 对应左臂 jid 5..11 / 右臂 jid 12..18。
# 左臂 j1..j7 控制器限位来自 20260827 只读回读[待归档]；j7 控制器虽报 +88°，
# 仍按 20260708 四次物理停止证据收紧到 +76°。右臂仍是 URDF 候选，待真机回读。
LIMITS_DEG = {
    1: [(-166.0, 166.0), (-10.0, 208.0), (-166.0, 166.0), (-128.0, 103.0),
        (-166.0, 166.0), (-40.0, 40.0), (-88.0, 76.0)],      # 左臂
...
def require_pypilot():
    if pypilot is None:
        raise RuntimeError(
            "当前 Python 环境缺少厂商 pypilot；可做 dry-run，真机会话必须在 Debian "
            "Python 3.10 SDK 容器内运行")
...
def open_gripper_session(robot_ip, local_ip, arm_ip, arm_port):
    """夹爪专用最小会话：不清报警、不使能、不设速度、不等待 Worlds。"""
    require_pypilot()
    sdk = pypilot.PilotSDK(robot_ip)
    started = False
    try:
        print("=" * 60); print("启动 PilotSDK(夹爪专用会话)"); print("=" * 60)
        if not sdk.start():
        ...
        if code != 0 or status != "CLOSE":
            raise RuntimeError(f"软急停状态异常: ({code}, {status})")
        code = sdk.armInitData(local_ip, arm_ip, arm_port)
        print("armInitData:", code); require_code_zero(code, "机械臂通信初始化")
        code, linked = sdk.armGetLinkStatus()
        print("link:", (code, linked)); require_true_status(code, linked, "机械臂通信链路")
        print("夹爪会话: 不清报警 / 不使能 / 不设速度 / 不读 armGetWorlds")
        return sdk
...
def open_read_only_session(robot_ip, local_ip, arm_ip, arm_port, arm_ids=(1,)):
    """不使能的只读会话；初始化任何阶段失败都会 sdk.stop()。"""
    require_pypilot()
    sdk = pypilot.PilotSDK(robot_ip)
    started = False
    try:
        print("=" * 60); print("启动 PilotSDK(只读会话)"); print("=" * 60)
        if not sdk.start():
            raise RuntimeError("PilotSDK 启动失败")
        started = True
        time.sleep(1.0)
        ...
        code, linked = sdk.armGetLinkStatus()
        print("link:", (code, linked)); require_true_status(code, linked, "机械臂通信链路")
        print("只读会话: 不清报警 / 不使能 / 不设全局速度 / 不下发运动")
        for attempt in range(1, 6):
            time.sleep(0.6)
            try:
                code, _vec = sdk.armGetWorlds(int(arm_ids[0]), True)
                if code == 0:
...
def open_session(robot_ip, local_ip, arm_ip, arm_port, global_speed, arm_ids=(1,),
    ...
    机械臂上电, 更不该顺手把它使能起来。"""
    if not enable:
        return open_read_only_session(robot_ip, local_ip, arm_ip, arm_port, arm_ids)
    require_pypilot()
    sdk = pypilot.PilotSDK(robot_ip)
    print("=" * 60)
    print("启动 PilotSDK")
    ...
    if not enable:  # 上方已转入 open_read_only_session；保留旧代码仅供历史对照。
    ...
    # 使能: 单次下发 + 等待(老版逻辑, 实测比"循环重发"稳 —— 反复重发会打断正在
    # 进行的使能过程。使能间歇失败多为机器人发热/上次残留, 属真机状态问题, 不在
    # 代码里硬重试; 失败就断电散热或重跑)。
...
def intersect_axis_limits(controller_limits, operational_limits):
    """控制器硬限位与本地已证实物理/运行包络取交集。

    控制器回读不能放宽更紧的物理停止证据（当前典型是左臂 j7 +76°）。若两张表
    无交集则立即报错，避免把配置漂移悄悄变成一张可执行限位表。
    """
    if len(controller_limits) != len(operational_limits):
        raise ValueError("controller/operational limits dimension mismatch")
    effective = []
    for index, ((controller_lo, controller_hi),
                (operational_lo, operational_hi)) in enumerate(
                    zip(controller_limits, operational_limits), 1):
        lo = max(float(controller_lo), float(operational_lo))
        hi = min(float(controller_hi), float(operational_hi))
        if lo >= hi:
            raise ValueError(
                f"j{index} controller/operational limits do not overlap: "
                f"controller=[{controller_lo},{controller_hi}] "
                f"operational=[{operational_lo},{operational_hi}]")
