...
用法:
    with robot_lock.acquire(robot_ip_from_runtime, 1, "execute_servo_grasp.py"):
        ...  # 这段期间独占该机械臂
"""
...
def robot_lock_path(robot_ip):
    """整机级锁路径；双臂串行视觉任务必须使用它。"""
    safe = str(robot_ip).replace(":", "_").replace("/", "_")
    return os.path.join(LOCK_DIR, f"robot_{safe}.lock")
...
def acquire(robot_ip, arm_id, owner="", required=True):
    ...
    # 旧单臂脚本持整机锁的共享锁；视觉双臂任务持排他锁。这样两类脚本也互斥，
    # 而左右两个旧单臂只读工具仍可并存（真实运动工具本身还有 arm 锁）。
    robot_path = robot_lock_path(robot_ip)
    robot_fh = open(robot_path, "a+")
    try:
        fcntl.flock(robot_fh.fileno(), fcntl.LOCK_SH | fcntl.LOCK_NB)
    except OSError as exc:
        robot_fh.close()
        if exc.errno in (errno.EACCES, errno.EAGAIN):
            raise SystemExit(f"✗ 机器人 {robot_ip} 正被双臂任务独占，单臂脚本拒绝启动")
        raise
    fh = open(path, "a+")
    try:
        try:
        ...
        fh.close()
        try:
            fcntl.flock(robot_fh.fileno(), fcntl.LOCK_UN)
        except Exception:                        # noqa: BLE001
            pass
        robot_fh.close()
...
@contextlib.contextmanager
def acquire_robot(robot_ip, owner="", required=True):
    """独占整台机器人，防止左右臂或工具脚本并发。"""
    path = robot_lock_path(robot_ip)
    if fcntl is None:
        msg = f"⚠ 本平台没有 fcntl, 无法加机器人级互斥锁 ({path})"
        if required:
            raise SystemExit(msg + " —— 拒绝在无锁保护下操作真机")
        print(msg + " —— 仅离线测试可继续")
        yield None
    ...
    try:
        try:
            ...
            try:
                ...
                held = {}
            raise SystemExit(
                f"✗ 机器人 {robot_ip} 正被另一个任务占用: "
                f"pid={held.get('pid','?')} {held.get('owner','?')} "
                f"起于 {held.get('since','?')}")
        fh.seek(0); fh.truncate()
        fh.write(json.dumps({"pid": os.getpid(), "owner": owner,
                             "since": time.strftime("%Y-%m-%d %H:%M:%S")},
                            ensure_ascii=False)); fh.flush()
        print(f"[机器人级互斥锁] 已独占 {robot_ip} (pid={os.getpid()})")
        yield fh
    finally:
        try:
            fcntl.flock(fh.fileno(), fcntl.LOCK_UN)
