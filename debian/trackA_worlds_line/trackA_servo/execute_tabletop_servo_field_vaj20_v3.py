...
# VAJ3 command-space limits.
VAJ3_VMAX_DEG_S = 20.0
VAJ3_AMAX_DEG_S2 = 1000.0
VAJ3_JMAX_DEG_S3 = 50000.0

# C2 spline must stay close to the verified q polyline.
VAJ3_SPLINE_TENSION = 0.45
VAJ3_CORRIDOR_DEG = 0.08

# Never allow another 300-second profile.
VAJ3_MAX_RUNTIME_S = 30.0
...
def field_stream(sdk, plan, log):
    ...
    runtime_plan, vaj3_stats = vaj3.retime_plan(
        runtime_plan,
        list(current["joints"]),
        dt=period_s,
        vmax=VAJ3_VMAX_DEG_S,
        amax=VAJ3_AMAX_DEG_S2,
        jmax=VAJ3_JMAX_DEG_S3,
        tension=VAJ3_SPLINE_TENSION,
        corridor_deg=VAJ3_CORRIDOR_DEG,
    )
    ...
    if total_duration_s > VAJ3_MAX_RUNTIME_S:
        raise RuntimeError(
            f"VAJ3 runtime {total_duration_s:.2f}s > "
            f"{VAJ3_MAX_RUNTIME_S:.2f}s guard"
        )

    if full_max_step_deg > RUNTIME_MAX_STEP_DEG + 1e-9:
        raise RuntimeError(
            f"VAJ3 max step {full_max_step_deg:.4f}° > "
            f"{RUNTIME_MAX_STEP_DEG:.4f}°"
        )

    log["field_runtime_servo"] = {
        "mode": "VAJ3_C2_SPLINE_JOINT_LIMITED",
        "seed_stride": RUNTIME_STRIDE,
        ...
        "command_limits": {
            "vmax_deg_s": VAJ3_VMAX_DEG_S,
            "amax_deg_s2": VAJ3_AMAX_DEG_S2,
            "jmax_deg_s3": VAJ3_JMAX_DEG_S3,
        },

        "command_measured": {
            "max_v_deg_s":
                vaj3_stats["max_v_deg_s"],
            "max_a_deg_s2":
                vaj3_stats["max_a_deg_s2"],
            "max_j_deg_s3":
                vaj3_stats["max_j_deg_s3"],
        },

        "spline": {
            "type": "C2_QUINTIC_HERMITE",
            "tension": VAJ3_SPLINE_TENSION,
            "corridor_limit_deg":
                VAJ3_CORRIDOR_DEG,
            "max_corridor_dev_deg":
                vaj3_stats["max_corridor_dev_deg"],
        },

        "vaj_block_count":
            vaj3_stats["block_count"],
        "vaj_blocks":
            vaj3_stats["blocks"],

        "scope":
            "C2_SPLINE_OVER_VERIFIED_Q_PATH_PLUS_JOINT_VAJ_RETIME",
    }

    print(
        f"FIELD SERVO VAJ3: "
        f"dt={period_s*1000:.0f}ms, "
        f"blocks={vaj3_stats['block_count']}, "
        f"frames={total_motion_frames}, "
        f"duration≈{total_duration_s:.2f}s, "
        f"step≤{full_max_step_deg:.3f}°, "
        f"v={vaj3_stats['max_v_deg_s']:.1f}/"
        f"{VAJ3_VMAX_DEG_S:.0f}, "
        f"a={vaj3_stats['max_a_deg_s2']:.1f}/"
        f"{VAJ3_AMAX_DEG_S2:.0f}, "
        f"j={vaj3_stats['max_j_deg_s3']:.0f}/"
        f"{VAJ3_JMAX_DEG_S3:.0f}, "
        f"corridor={vaj3_stats['max_corridor_dev_deg']:.4f}°/"
        f"{VAJ3_CORRIDOR_DEG:.2f}°",
        flush=True,
    )
