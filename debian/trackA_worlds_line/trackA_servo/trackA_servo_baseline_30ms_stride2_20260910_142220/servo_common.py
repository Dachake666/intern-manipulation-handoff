...
class StrictPacer:
    ...
    def before_send(self):
        ...
        if self.due is not None:
            ...
            if late > 0.0:
                self.late_frames += 1
                self.total_late_s += late

            if late > 2*self.period:
                # Field/runtime policy:
                # 不追补积压帧，也不因一次 SDK/系统调度抖动停止整条 Servo。
                # 本 StrictPacer 本来每次都基于当前 now 生成下一 due，
                # 因此这里记录一次 realign 后直接继续即可。
                self.realigns += 1
    ...
    def report(self):
        return {
            "send_slots": self.frames,
            "period_s": self.period,
            "late_frames": self.late_frames,
            "late_ratio": (
                self.late_frames / self.frames
                if self.frames else 0.0
            ),
            "max_late_ms": self.max_late_s * 1000.0,
            "mean_late_ms": (
                self.total_late_s / self.late_frames * 1000.0
                if self.late_frames else 0.0
            ),
            "realigns": self.realigns,
            "catchup_bursts_allowed": False,
