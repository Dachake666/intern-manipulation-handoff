...
# [重建 20260905] 现场摘要的旧计划容差；启用恢复的新计划使用 ARRIVE 严格容差。
...
RESOLVED_STAGE_KEYS = {
    ...
    "PICK_ASCEND": "pick_ascend_endpoint",
...
def _finite_vector(values: Any, length: int, label: str) -> list[float]:
    ...
    if any(isinstance(value, bool) for value in values):
        raise PlanError(f"{label} 不接受布尔值")
    try:
...
def validate_plan(plan: Mapping[str, Any]) -> None:
    ...
    if ("pick_mode" in plan or "startup_policy" in plan) and safety.get("input_geometry_gate") != (
            "BLOCKED" if blockers else "PASS"):
        raise PlanError("新计划的 input_geometry_gate 必须与 input_blockers 一致")
    ...
    resolved_poses["pick_ascend_endpoint"] = _pose6(
        resolved.get("pick_ascend_endpoint", resolved["pick_hover_endpoint"]),
        "resolved.pick_ascend_endpoint")
    validate_startup_policy(plan.get("startup_policy"))
    ...
    # 老 JSON 不含 pick_mode 时沿用竖直抓取；新水平方案明确锁 Y/Z。
    pick_mode = plan.get("pick_mode", "VERTICAL")
    if pick_mode not in ("VERTICAL", "HORIZONTAL_X_POSITIVE"):
        raise PlanError("未知 pick_mode")
    pick, approach, ascend = (resolved_poses[key] for key in
                              ("pick_endpoint", "pick_hover_endpoint", "pick_ascend_endpoint"))
    if pick_mode == "HORIZONTAL_X_POSITIVE":
        if "pick_ascend_endpoint" not in resolved:
            raise PlanError("水平抓取必须提供独立 pick_ascend_endpoint")
        if not _same_pose(approach[1:3], pick[1:3]) or approach[0] >= pick[0]:
            raise PlanError("水平抓取必须从 X- 沿 +X 接近，Y/Z 必须固定")
    elif not _same_pose(approach[:2], pick[:2]) or approach[2] <= pick[2]:
        raise PlanError("竖直抓取接近点必须在抓点正上方")
    if not _same_pose(ascend[:2], pick[:2]) or ascend[2] <= pick[2]:
        raise PlanError("PICK_ASCEND 必须同 XY 沿 +Z 抬升")


def validate_startup_policy(policy: Any) -> None:
    """只用标准库，Mac 与 Debian 共用同一个恢复契约。缺省不自动回位。"""
    if policy is None:
        return
    if not isinstance(policy, Mapping):
        raise PlanError("startup_policy 必须是对象")
    if policy.get("mode") == "REQUIRE_SAFE":
        if set(policy) != {"mode"}:
            raise PlanError("REQUIRE_SAFE 不接受其他恢复参数")
        return
    required = {"mode", "clearance_z_mm", "allowed_start_xyz_bounds_mm",
                "maximum_rotation_deg", "safe_joints_deg", "safe_joint_tolerance_deg",
                "endpoint_exclusion_boxes_mm", "qualification"}
    if set(policy) != required or policy.get("mode") != "RECOVER_TO_TASK_SAFE_IF_NEEDED":
        raise PlanError("startup_policy 字段缺失或含未知字段")
    z, rotation, tolerance = _finite_vector([
        policy["clearance_z_mm"], policy["maximum_rotation_deg"],
        policy["safe_joint_tolerance_deg"]], 3, "startup policy scalars")
    if not 0 < rotation <= 45 or not 0 < tolerance <= MAX_JOINT_STEP_DEG:
        raise PlanError("恢复转角须在 (0,45]°，safe 关节容差不得超过相邻 IK 上限")
    bounds = policy["allowed_start_xyz_bounds_mm"]
    if not isinstance(bounds, list) or len(bounds) != 3:
        raise PlanError("恢复区域必须为 XYZ 三对 [min,max]，单位 mm")
    for pair in bounds:
        lo, hi = _finite_vector(pair, 2, "恢复区域")
        if lo >= hi:
            raise PlanError("恢复区域必须 min < max")
    if not bounds[2][0] <= z <= bounds[2][1]:
        raise PlanError("恢复高度必须位于恢复区域 Z 范围内")
    _finite_vector(policy["safe_joints_deg"], 7, "safe_joints_deg")
    boxes = policy["endpoint_exclusion_boxes_mm"]
    if not isinstance(boxes, list):
        raise PlanError("endpoint_exclusion_boxes_mm 必须是数组")
    for box in boxes:
        if not isinstance(box, Mapping) or set(box) != {"name", "min_mm", "max_mm"}:
            raise PlanError("恢复禁入盒必须含 name/min_mm/max_mm")
        if not isinstance(box["name"], str) or not box["name"].strip():
            raise PlanError("恢复禁入盒 name 不能为空")
        lo = _finite_vector(box["min_mm"], 3, "exclusion.min_mm")
        hi = _finite_vector(box["max_mm"], 3, "exclusion.max_mm")
        if any(a >= b for a, b in zip(lo, hi)):
            raise PlanError("恢复禁入盒必须有正体积")
    evidence = policy["qualification"]
    if not isinstance(evidence, Mapping) or set(evidence) != {"status", "scope", "source", "scene_revision"}:
        raise PlanError("恢复 qualification 缺失或字段不正确")
    if evidence["status"] not in ("PENDING", "CONFIRMED") or evidence["scope"] != "FULL_ARM_EMPTY_GRIPPER_RECOVERY_VOLUME":
        raise PlanError("恢复资格必须覆盖整臂及空夹爪的整个恢复区域")
    if any(not isinstance(evidence[k], str) or not evidence[k].strip()
           for k in ("source", "scene_revision")):
        raise PlanError("恢复资格必须记录来源与场景版本")
    if evidence["status"] == "CONFIRMED" and not boxes:
        raise PlanError("已确认恢复区域必须有包含工具包络余量的环境禁入盒")


def recovery_enabled(plan: Mapping[str, Any]) -> bool:
    return (plan.get("startup_policy") or {}).get("mode") == "RECOVER_TO_TASK_SAFE_IF_NEEDED"


def build_startup_recovery(current: Sequence[float], plan: Mapping[str, Any]) -> list[tuple[str, list[float]]]:
    """只计算路点；PENDING 区域可用于只读 IK 诊断，不能用于真机恢复。"""
    current = _finite_vector(current, 6, "current worlds")
    safe = _pose6(plan["resolved"]["safe_endpoint"], "safe")
    if not recovery_enabled(plan):
        assert_current_safe(current, safe)
        return []
    try:
        assert_current_safe(current, safe, strict=True)
        return []
    except PrecheckError:
        pass
    policy = plan["startup_policy"]
    for label, point in (("当前位置", current), ("safe", safe)):
        if any(not lo <= value <= hi for value, (lo, hi) in
               zip(point[:3], policy["allowed_start_xyz_bounds_mm"])):
            raise PrecheckError(f"{label} 超出允许自动恢复区域；需人工示教回安全区")
    if max(_angular_error_deg(a, b) for a, b in zip(current[3:], safe[3:])) > policy["maximum_rotation_deg"]:
        raise PrecheckError("当前姿态偏差超过自动恢复范围；需人工示教")
    height = max(current[2], safe[2], policy["clearance_z_mm"])
    targets = [
        ("RECOVERY_LIFT", current[:2] + [height] + current[3:]),
        ("RECOVERY_ROTATE", current[:2] + [height] + safe[3:]),
        ("RECOVERY_TRANSIT", safe[:2] + [height] + safe[3:]),
        ("RECOVERY_DESCEND", safe),
    ]
    result = []
    previous = current
    for name, target in targets:
        if not _same_pose(previous, target):
            result.append((name, target))
            previous = target
    # 连续线段与已扩张的禁入 AABB 相交检测，不以稀疏采样替代。
    previous = current
    for name, target in result:
        for box in policy["endpoint_exclusion_boxes_mm"]:
            t_min, t_max = 0.0, 1.0
            for a, b, lo, hi in zip(previous[:3], target[:3], box["min_mm"], box["max_mm"]):
                delta = b - a
                if abs(delta) < 1e-12:
                    if not lo <= a <= hi:
                        t_min, t_max = 1.0, 0.0
                    ...
                    u, v = sorted(((lo - a) / delta, (hi - a) / delta))
                    t_min, t_max = max(t_min, u), min(t_max, v)
            if t_min <= t_max:
                raise PrecheckError(f"{name} 穿过恢复禁入区 {box['name']}")
        previous = target
    return result


def run_gate_error(plan: Mapping[str, Any], run_requested: bool,
                   allow_real_motion: bool, *, recovery_only: bool = False) -> str | None:
    if not run_requested:
        return None
    blockers = plan["safety"]["input_blockers"]
    if not allow_real_motion:
        return "--run 已给出，但 XIFENG_ALLOW_REAL_MOTION!=1"
    if recovery_only and recovery_enabled(plan):
        return None  # 仅回位不执行抓放；动态恢复的资格由 live 起点路径检查阻断。
...
def assert_current_safe(current: Sequence[float], safe: Sequence[float], *, strict: bool = False) -> None:
    ...
    position_tol = ARRIVE_TOL_MM if strict else SAFE_POSITION_TOL_MM
    orientation_tol = ARRIVE_TOL_DEG if strict else SAFE_ORIENTATION_TOL_DEG
    if position_error > position_tol or orientation_error > orientation_tol:
        raise PrecheckError(
            "当前 endpoint 不在计划 safe 点："
            f"位置最大差 {position_error:.2f}mm（允许 {position_tol:.1f}mm），"
            f"姿态最大差 {orientation_error:.2f}°（允许 {orientation_tol:.1f}°）")
...
def _controller_limit_violations(
        joints: Sequence[float], limits: Sequence[Sequence[float]], *, margin: float | None = None) -> list[str]:
    margin = LIMIT_MARGIN_DEG if margin is None else margin
    ...
    for axis, (q, pair) in enumerate(zip(joints, limits), 1):
        ...
        if not lo + margin <= float(q) <= hi - margin:
            bad.append(
...
def precheck_full_path(ss: Any, sdk: Any, plan: Mapping[str, Any], *,
                       margin: float = LIMIT_MARGIN_DEG, recovery_only: bool = False,
                       expected_start: Mapping[str, Any] | None = None) -> dict[str, Any]:
    """检查 current≈safe、双重限位及每条 MoveWorlds 直线的密集 IK 连续性。"""
    safe = _pose6(plan["resolved"]["safe_endpoint"], "resolved.safe_endpoint")
    current_world = _finite_vector(ss.read_worlds(sdk, ARM_ID), 6, "当前 Worlds")
    current_joints = _finite_vector(ss.read_joints(sdk, ARM_ID), 7, "当前关节")
    print(f"live_start: worlds={current_world}; joints={current_joints}")
    if expected_start is not None:
        assert_current_safe(current_world, expected_start["current_world"], strict=True)
        if max(abs(a-b) for a,b in zip(current_joints, expected_start["current_joints_deg"])) > ARRIVE_TOL_DEG:
            raise PrecheckError("两次预检之间关节位置改变，请重新启动预检")
    recovery = build_startup_recovery(current_world, plan)
    if recovery_enabled(plan) and not recovery:
        _assert_safe_branch(current_joints, plan)
    try:
    ...
    bad = list(ss.check_limits(
        ARM_ID, current_joints, margin=margin))
    bad += _controller_limit_violations(current_joints, controller_limits, margin=margin)
    if bad:
    ...
    total_dense = 0
    targets = recovery + ([] if recovery_only else movement_targets(plan))
    used_min = current_joints[:]
    used_max = current_joints[:]
    for segment_name, target in targets:
        ...
        for sample_index, pose in enumerate(dense, 1):
            ...
            joints = _finite_vector(joints, 7, f"{segment_name} IK")
            used_min = [min(a,b) for a,b in zip(used_min,joints)]
            used_max = [max(a,b) for a,b in zip(used_max,joints)]
            bad = list(ss.check_limits(
                ARM_ID, joints, margin=margin))
            bad += _controller_limit_violations(joints, controller_limits, margin=margin)
            if bad:
                raise PrecheckError(
                    f"{segment_name} 密集点 {sample_index}/{len(dense)} 限位失败:\n  " +
                    "\n  ".join(bad))
            deltas = [abs(a - b) for a, b in zip(joints, previous_joints)]
            jump = max(deltas)
            maximum_jump = max(maximum_jump, jump)
            if jump > MAX_JOINT_STEP_DEG:
                raise PrecheckError(
                    f"{segment_name} 密集点 {sample_index}/{len(dense)} "
                    f"IK 跳变 J{deltas.index(jump)+1}: {jump:.2f}° > {MAX_JOINT_STEP_DEG:.2f}°; "
                    f"pose={pose}; previous_joints={previous_joints}; joints={joints}")
        ...
        if recovery_enabled(plan) and _same_pose(target, safe):
            _assert_safe_branch(previous_joints, plan)
        print(f"  {segment_name}: dense={len(dense)} max_joint_step={maximum_jump:.3f}deg target={target}")
    return {
        ...
        "joint_used_ranges_deg": list(map(list, zip(used_min, used_max))),
        "recovery_qualification": (plan["startup_policy"]["qualification"]["status"]
                                    if recovery_enabled(plan) else "NOT_APPLICABLE"),
        "collision_scope": "ENDPOINT_EXCLUSIONS_PLUS_EXTERNAL_RECOVERY_VOLUME_QUALIFICATION",
        "task_collision_status": plan.get("safety", {}).get("robot_gripper_held_object_collision_preflight", "PENDING"),
    }


def _assert_safe_branch(joints: Sequence[float], plan: Mapping[str, Any]) -> None:
    policy = plan["startup_policy"]
    if max(abs(a-b) for a,b in zip(joints, policy["safe_joints_deg"])) > policy["safe_joint_tolerance_deg"]:
        raise PrecheckError("safe 末端位置相同但关节分支不同；拒绝继续抓放")


def execute_recovery(ss: Any, sdk: Any, plan: Mapping[str, Any], report: Mapping[str, Any]) -> None:
    for name, target in report["recovery_targets"]:
        print(f"[{name}] MOVE_WORLDS target={target}")
        ss.check_soft_stop(sdk, name)
        ss.move_worlds(sdk, ARM_ID, target, interpolation_en=False)
        ss.wait_until_worlds(sdk, ARM_ID, target, tol_mm=ARRIVE_TOL_MM,
                            tol_deg=ARRIVE_TOL_DEG, timeout_s=ARRIVE_TIMEOUT_S, label=name)
    if recovery_enabled(plan):
        assert_current_safe(ss.read_worlds(sdk, ARM_ID),
                            _pose6(plan["resolved"]["safe_endpoint"], "safe"), strict=True)
        _assert_safe_branch(_finite_vector(ss.read_joints(sdk, ARM_ID), 7, "safe joints"), plan)
...
def execute_stages(ss: Any, sdk: Any, plan: Mapping[str, Any]) -> None:
    ...
    for stage in plan["stages"]:
        ...
        if kind == "ASSERT_ENDPOINT":
            assert_current_safe(ss.read_worlds(sdk, ARM_ID), safe, strict=recovery_enabled(plan))
        elif kind == "GRIPPER":
...
def run_with_sdk(ss: Any, plan: Mapping[str, Any], config: Mapping[str, Any], *,
    ...
    validate_plan(plan)
    margin = config.get("limit_margin_deg", LIMIT_MARGIN_DEG)
    recovery_only = config.get("recovery_only", False)
    if real_motion:
        error = run_gate_error(plan, True, True, recovery_only=recovery_only)
        if error:
            raise PrecheckError(error)

    # 第一遍永远是只读 live 预检。失败时从未使能、未开串口、未运动。
    readonly_sdk = None
    try:
        readonly_sdk = _open_session(ss, config, enable=False)
        report = precheck_full_path(ss, readonly_sdk, plan, margin=margin,
                                   recovery_only=recovery_only)
        print(
            f"只读预检通过：{len(report['segments'])} 条 MoveWorlds 直线，"
            f"{report['dense_points']} 个 armTryWorlds 密集点。")
        print(f"current_world={report['current_world']}; current_joints={report['current_joints_deg']}")
        for name, target in report["recovery_targets"]:
            print(f"  拟恢复 {name}: {target}")
        print(f"限位余量 {margin}°；实时控制器限位检查开启；armTryWorlds 不证明碰撞净空。")
    finally:
    ...
    if not real_motion:
        if report["recovery_targets"]:
            print(f"恢复区域资格: {plan['startup_policy']['qualification']['status']}")
        print("DRY RUN：只读会话已关闭；零使能、零运动、零夹爪命令。")
        return 0
    if report["recovery_targets"] and plan["startup_policy"]["qualification"]["status"] != "CONFIRMED":
        raise PrecheckError("恢复区域整臂/空夹爪净空尚未确认；仅完成 IK 诊断，未使能。请先确认当前场景恢复区域")
    if report["recovery_targets"]:
        answer = input_fn("将执行上述回 safe 路段。确认夹爪为空、场景与已确认恢复区域一致，输入 EMPTY 继续: ")
        if answer.strip() != "EMPTY":
            print("未确认空夹爪；不使能、不恢复。")
            return 2
    answer = input_fn(
    ...
    try:
        ...
        report = precheck_full_path(ss, sdk, plan, margin=margin,
                                   recovery_only=recovery_only, expected_start=report)
        print(
            f"执行前复检通过：{report['dense_points']} 个密集 IK 点；"
            "先执行需要的启动回位。")
        execute_recovery(ss, sdk, plan, report)
        if recovery_only:
            print("仅回 safe 测试完成，未打开 COM1、未操作夹爪、未开始抓放。")
            return 0
        if report["recovery_targets"]:
            post_recovery = precheck_full_path(ss, sdk, plan, margin=margin)
            if post_recovery["recovery_targets"]:
                raise PrecheckError("回位后偏离 safe，不重试自动回位")
...
def _runtime_config(args: argparse.Namespace, environ: Mapping[str, str]) -> dict[str, Any]:
    ...
    if not math.isfinite(margin) or not 0 <= margin <= LIMIT_MARGIN_DEG:
        raise PlanError(f"--limit-margin-deg 必须在 [0,{LIMIT_MARGIN_DEG}]；保留实时控制器/物理限位")
    return {
        "robot_ip": robot_ip,
        "local_ip": local_ip,
        "arm_ip": arm_ip,
...
def main(argv: Sequence[str] | None = None, *, ss_module: Any = None,
    ...
    parser.add_argument("--speed", type=float, default=GLOBAL_SPEED,
                        help="真机全局速度百分比，范围 [0.1,20.0]，默认 1.0；新路径建议 1~5")
    parser.add_argument("--limit-margin-deg", type=float, default=LIMIT_MARGIN_DEG,
                        help="限位内缩余量，默认 5°；0 仍检查实时控制器限位与 j7 物理限位")
    parser.add_argument("--recovery-only", action="store_true",
                        help="只预检/执行启动回 safe；不打开串口、不抓放")
    args = parser.parse_args(argv)
    env = os.environ if environ is None else environ

    try:
        plan, plan_sha256 = load_plan(args.plan)
        gate_error = run_gate_error(plan, args.run,
                                   env.get("XIFENG_ALLOW_REAL_MOTION") == "1",
                                   recovery_only=args.recovery_only)
        if gate_error:
    ...
    with lock_module.acquire(
        ...
        except (PrecheckError, PlanError) as exc:
            print(f"预检/执行中止：{exc}")
            return 2
