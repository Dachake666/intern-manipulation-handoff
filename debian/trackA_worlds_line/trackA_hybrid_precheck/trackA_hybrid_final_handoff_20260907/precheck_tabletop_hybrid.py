...
def validate(plan):
    ...
    if (plan.get("schema_version") != SCHEMA or plan.get("status") != "OFFLINE_CANDIDATE_BLOCKED"
    ...
    for frame in frames:
        ...
        elif name != "CAPTURED_LIVE_START_NOT_ASSERTED_SAFE" or phase != -1:
...
def capture(helper, config, plan, report):
    ...
    try:
        ...
        if sdk is not None:
            try:
                sdk.stop()  # 未改变速度/保护，无须通用close_session中的控制器写操作。
