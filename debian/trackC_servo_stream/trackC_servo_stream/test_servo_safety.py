...
class TestSession(unittest.TestCase):
    ...
    def test_enable_settles_by_readonly_poll_without_resend(self):
        """前两次状态 False 也只发一次使能，等 getter 自己变 True。"""
        log = []
        calls = {"status": 0}
        import pypilot
        SDK = self._sdk(log)

        def delayed_status(_self):
            calls["status"] += 1
            return (0, calls["status"] >= 3)

        SDK.armGetRobotEnableStatus = delayed_status
        pypilot.PilotSDK = SDK
        with mock.patch.object(ss.time, "sleep", return_value=None):
            with contextlib.redirect_stdout(io.StringIO()):
                sdk = ss.open_session("ip", "l", "a", 1, 3.0, (1,))
                ss.close_session(sdk)
        self.assertEqual(log.count("使能"), 1)
        self.assertEqual(calls["status"], 3)

    def test_enable_timeout_never_resends_and_still_aborts(self):
        """持续 False 时只做完只读检查并中止，不能绕过安全门。"""
        log = []
        calls = {"status": 0}
        import pypilot
        SDK = self._sdk(log)

        def false_status(_self):
            calls["status"] += 1
            return (0, False)

        SDK.armGetRobotEnableStatus = false_status
        pypilot.PilotSDK = SDK
        with mock.patch.object(ss.time, "sleep", return_value=None):
            with contextlib.redirect_stdout(io.StringIO()):
                with self.assertRaises(RuntimeError):
                    ss.open_session("ip", "l", "a", 1, 3.0, (1,))
        self.assertEqual(log.count("使能"), 1)
        # 10 次等待检查 + 超时诊断里再读 1 次。
        self.assertEqual(calls["status"], ss.ENABLE_STATUS_POLLS + 1)
